-- test_routing_split.sql — uji rute Bintaro → SCBD di tabel HASIL SPLIT.
-- Prasyarat: split_java.sql selesai (java_ways_split bertopologi + biaya).
-- Jalankan: psql -h 172.80.1.2 -U postgres -d geomapping_id -f test_routing_split.sql
...
FROM java_ways_split_vertices_pgr v
...
WITH scbd_way AS (
    SELECT id, geom FROM java_ways_split
...
FROM java_ways_split_vertices_pgr v, scbd_way w
...
FROM java_ways_components_split
...
FROM pgr_dijkstra(
    'SELECT id, source, target, cost, reverse_cost FROM java_ways_split',
    :bintaro_vid, :scbd_vid, directed := true
) r
JOIN java_ways_split w ON w.id = r.edge;
